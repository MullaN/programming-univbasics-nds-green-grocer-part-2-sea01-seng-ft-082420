  ...
  coupons.each do |coupon_item|
    ...
    if cart_num
      if cart[cart_num][:count] >= coupon_item[:num]
        cart << {item: "#{cart[cart_num][:item]} W/ COUPON", price: coupon_item[:cost] / coupon_item[:num], clearance: cart[cart_num][:clearance], count: (cart[cart_num][:count] / coupon_item[:num]).to_i}
        binding.pry
      end
    end
  end
